...
def main(path_surface: str, path_filaments: str, path_poles: str,
         pixel_size: float, gap_size: int, kmts_to_surface_distance: int,
         visualize: bool):
    ...
    am = NumpyToAmira(as_point_cloud=False,
                      header=[
    ...
    if visualize:
        from tardis_em.utils.visualize_pc import VisualizeFilaments, VisualizeSurface, VisualizeCompose
        vertices, triangles = classifier.get_vertices(simplify=128)
        vis_kmt = VisualizeFilaments(np.vstack((np.vstack(classes[0]), np.vstack(classes[1]))),
                                     False, filament_color="red", return_=True)
        vis_mid_mt = VisualizeFilaments(classes[2], False, filament_color="magenta", return_=True)
        vis_brg_mt = VisualizeFilaments(classes[3], False, filament_color="white", return_=True)
        vis_int_mt = VisualizeFilaments(classes[4], False, filament_color="orange", return_=True)
        vis_smt = VisualizeFilaments(classes[5], False, filament_color="yellow", return_=True)
        surf = VisualizeSurface(vertices, triangles, False, return_=True)

        VisualizeCompose(False,
                         vis_smt=vis_smt,
                         vis_kmt=vis_kmt,
