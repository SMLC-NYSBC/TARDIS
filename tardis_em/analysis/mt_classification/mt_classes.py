...
class MicrotubuleClassifier:
    ...
    def correct_data(self):
        ...
        min_coords = self.filaments[:, 1:].min(axis=0)

        # Efficient normalization using broadcasting
        self.filaments[:, 1:] = (self.filaments[:, 1:] - min_coords) / 25.70
        self.filaments = resample_filament(self.filaments, 1)

        # Normalize vertices and poles
        for i in range(len(self.vertices)):
            self.vertices[i] = (self.vertices[i] - min_coords) / self.pixel_size

        self.poles = ((self.poles - min_coords) / self.pixel_size).astype(np.int32)
        self.poles = pick_pole_to_surfaces(self.poles, self.vertices)

        # Assign filaments to poles and reorder
        self.filament_pole1, self.filament_pole2 = assign_filaments_to_poles(self.filaments, self.poles)
    ...
    def assign_to_kmts(self, filaments, id_=0):
        ...
        d1_, d2_ = distances_of_ends_to_surface(self.vertices[id_], self.poles[id_], plus_end)
    ...
    def classified_MTs(self):
        ...
        self.kmts_inside_id_1, self.kmts_outside_id_1 = list(self.kmts_inside_id_1), list(self.kmts_outside_id_1)
        ...
        self.kmts_outside_id_2, self.kmts_inside_id_2 = list(self.kmts_outside_id_2), list(self.kmts_inside_id_2)
        ...
        """Select SMTs"""
        self.smt_ids = self.kmts_id_1 + self.kmts_id_2 + self.mid_mt_ids + self.int_mt_ids + self.brg_mt_ids
    ...
    def get_vertices(self, simplify=128):
        if simplify is not None:
            ...
            _, _, vertices, self.triangles = self.get_vertices_file(self.surfaces, simplify)
