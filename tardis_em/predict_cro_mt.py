...
from tardis_em.utils.normalization import MinMaxNormalize, RescaleNormalize
...
def main(
    ...
    output = join(path, "temp", "Predictions")
    am_output = join(path, "Predictions")

    predict_list = [f for f in listdir(path) if f.endswith(available_format)]
    ...
    if len(predict_list) == 0:
        tardis_progress(
            ...
            text_9=f"Given {path} is does not contain any recognizable file formats!",
        )
        sys.exit()
    else:
        tardis_progress(
    ...
    if np.any([True for x in predict_list if x.endswith((".tif", ".tiff"))]):
        tif_px = click.prompt(
            "Detected .tif files, please provide pixel size:", type=float
        )
    ...
    tardis_progress = TardisLogo()
    for id_, i in enumerate(sorted(predict_list)):
        """Pre-Processing"""
        ...
        tardis_progress(
            title=f"Fully-automatic MT segmentation module  {str_debug}",
            text_1=f"Found {len(predict_list)} images to predict!",
            text_3=f"Image {id_ + 1}/{len(predict_list)}: {i}",
        ...
        # Build temp dir
        build_temp_dir(dir_=path)

        # Cut image for smaller image
        if i.endswith(".am"):
            image, px, _, transformation = import_am(am_file=join(path, i))
        else:
            image, px = load_image(join(path, i))
        ...
        if (
            not np.min(image) >= 0 or not np.max(image) <= 1
        ...
        patches_DL = PredictionDataset(img_dir=join(path, "temp", "Patches"))
        ...
        for j in range(len(patches_DL)):
            ...
            # Pick image['s]
            input_, name = patches_DL.__getitem__(j)

            if j == 0:
                start = time.time()

                # Predict & Threshold
                input_ = predict_cnn.predict(input_[None, :])
            ...
            if cnn_threshold != 0:
                input_ = np.where(input_ >= cnn_threshold, 1, 0)

            tif.imwrite(
                join(output, f"{name}.tif"), np.array(input_, dtype=input_.dtype)
            )
        ...
        image = image_stitcher(image_dir=output, mask=True, dtype=input_.dtype)[
        ...
        for id_dist, coord in enumerate(coords_df):
            if id_dist % iter_time == 0:
                tardis_progress(
                    ...
                    text_5=f"Point Cloud: {point_cloud.shape[0]} Nodes; NaN Segments",
                    text_7="Current Task: DIST prediction...",
                    text_8=print_progress_bar(id_, len(coords_df)),
                )
        ...
        if debug:
            if device == "cpu":
                np.save(
                    join(am_output, f"{i[:-out_format]}_coord_voxel.npy"), point_cloud
                )
        ...
        if output == "csv":
            np.savetxt(
                join(am_output, f"{i[:-out_format]}" "_SpatialGraph.csv"),
                segments,
                delimiter=",",
            )
            np.savetxt(
