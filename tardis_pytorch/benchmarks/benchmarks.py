...
from tardis_pytorch.benchmarks.predictor import CnnBenchmark, DISTBenchmark
from tardis_pytorch.utils.aws import get_benchmark_aws, put_benchmark_aws
...
from tardis_pytorch.utils.metrics import compare_dict_metrics
...
def main(
    ...
    model = torch.load(model_checkpoint, map_location=get_device(device))
    ...
    if data_set not in listdir(join(DIR_, "Eval_DIST")):
        TardisError(
            id="",
            py="tardis_pytorch/benchmarks/benchmarks.py",
    ...
    link = "https://tardis-weigths.s3.amazonaws.com/benchmark/models/" f"{id_save}"
