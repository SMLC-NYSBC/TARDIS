...
from tardis_pytorch.spindletorch.utils.utils import scale_image
...
class DataSetPredictor:
    ...
    def __init__(
        ...
        # Check for spatial graph in folder from amira/tardis_pytorch comp.
...
class Predictor:
    ...
    def __init__(
        ...
        if checkpoint is None and network is None:
            TardisError(
                "139",
                "tardis_pytorch/utils/predictor.py",
            ...
            if self.network == "dist":
                ...
                if rotate:
                    if self._2d:
                        ...
                        out = torch.mean(torch.stack(rotate_x, dim=0), dim=0).numpy()
                else:
