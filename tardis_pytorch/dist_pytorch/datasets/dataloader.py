...
class ScannetColorDataset(BasicDataset):
    ...
    def __getitem__(self, i: int) -> Tuple[list, list, list, list, list]:
        ...
        if not isdir(self.color_dir):
            TardisError(
                "12",
                "tardis_pytorch/dist_pytorch/datasets/dataloader.py",
        ...
        if self.patch_size[i, 0] == 0:
            ...
            if self.rgb:
                (
                    ...
                    output_idx,
                    cls_idx,
                ) = self.VD.patched_dataset(
                    coord=coord, rgb=rgb_v, mesh=8, random=False
                )
            else:
                (
                    coords_idx,
                    ...
                    output_idx,
                    cls_idx,
                ) = self.VD.patched_dataset(coord=coord, mesh=8, random=False)
