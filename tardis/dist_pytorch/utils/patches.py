...
class PatchDataSet:
    ...
    Class for computing optimal patch size for given max number of points per patch

    Args:
        coord: np.ndarray (N, (2,3))
                2D or 3D array of the point cloud
        label_cls: np.ndarray (N, 1)
            default: None
                Optional class id array for each point in the point cloud
        rgb: np.ndarray (N, 3)
            default: None
                Optional rgb feature array for each point in the point cloud
        patch_3d: boolean
            default: False
                If True, compute patches in 3D. If False, patches are computed
                in 2D and if coord (N, 3), Z dimension is np.inf
        max_number_of_points: int
            default: 500
                Maximal allowed number of points per patch
        init_patch_size: float
            default: 0
                Initial patch size. If 0, initial patch size is taken as highest 
                value from computed boundary box
        drop_rate: float
            default: 0
                Optimizer step size for reducing size of patches
        graph: boolean
            default: True
                If True output computed graph for each patch of point cloud
        tensor: boolean
            default: True
                If True output all datasets as torch.Tensor

    Returns
    -------
        coord_patch: list of arrays
            List of arrays (N, 2) or (N, 3) with coordinates of points per patch
        rgb_patch:   list of arrays
            List of array (N, 3) with RGB value for each points peer patch
        graph_patch: list of arrays
            Optional list of all computed graphs fro each coord_patch
        output_idx:  list of arrays
            List of array (N, 1) with ordered ID value for each points peer patch.
            Ordered ID value allow to reconstruct point cloud from patches
        cls_patch:   list of arrays
            List of array (N, 3) with classes id  for each points peer patch
    """
    ...
    def _boundary_box(self):
        """
        Utile class function to compute boundary box

        Return
        ------
        boundary_box: np.ndarray (2, 3)
    ...
    def _collect_patch_idx(self,
        ...
        Args:
            patches: np.ndarray
                List of all patches centers

        Return
        ------
        not_empty_patch: list
            IDs list of all patches with more then 1 point
        points_no: list
            List of point number in each patch
        """
        not_empty_patch = []
        points_no = []
    ...
    @staticmethod
    def _normalize_idx(coord_with_idx: np.ndarray):
        """
        Utile class function to replace ids with ordered output ID value for 
        each point in patches

        Args:
            coord_with_idx: np.ndarray
                Coordinate id value i
        Return
        ------
        coord_with_idx: np.ndarray (N, (2, 3))
            Array of all point in patch with corrected ID value
        """
    ...
    def _output_format(self,
                       data: np.ndarray):
        """
        Utile class function to output array in correct format (numpy or tensor)

        Args:
            data: np.ndarray
                Input data for format change
    ...
    def _patch_centers(self,
        ...
        Args:
            boundary_box: np.ndarray
                Computer point cloud boundary box

        Return
        ------
        patch: np.ndarray: (N, 3)
                Array with XYZ coordinates to localize patch centers
        """
        patch = []
        patch_positions_x = []
        patch_positions_y = []
    ...
    def _points_in_patch(self,
        ...
        """
        Utile class function for filtering point cloud

        Args:
            patch_center: np.ndarray
                Array (1, 3) for given patch center

        Return
        ------
        coord_idx: boolean array
                Array of all points that are enclose in given patch
        """
    ...
    def optimize_patch_size(self):
        ...
        Function take __init__ stored variable and iteratively search for patch size
        small enough that allow for all patches to have equal or less max number 
        of points.
        """
    ...
    def patched_dataset(self,
        ...
        Args:
            mesh: boolean
                If True, build graph for meshes not filaments
            dist_th: float
                Distance threshold for graph from meshes
        """
        coord_patch = []
        graph_patch = []
        output_idx = []
        ...
        if self.coord.shape[0] <= self.DOWNSAMPLING_TH:
            ...
            if self.GRAPH_OUTPUT:
                ...
                build_graph = BuildGraph(coord=coord_label,
                                         mesh=mesh)
                if mesh:
                    graph_patch.append(self._output_format(build_graph(dist_th)))
                else:
                    graph_patch.append(self._output_format(build_graph()))
            ...
            for i in all_patch:
                ...
                if self.GRAPH_OUTPUT:
                    ...
                    build_graph = BuildGraph(coord=segment_patch,
                                             mesh=mesh)
                    if mesh:
                        graph_patch.append(self._output_format(build_graph(dist_th)))
                    else:
