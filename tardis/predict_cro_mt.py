...
def main(
    ...
    if np.any([True for x in predict_list if x.endswith((".tif", ".tiff"))]):
        tif_px = click.prompt("Detected .tif files, please provide pixel size:", type=float)
    ...
    for id, i in enumerate(sorted(predict_list)):
        ...
        segments = GraphToSegment.patch_to_segment(
            graph=graphs, coord=point_cloud, idx=output_idx, prune=5, visualize=visualizer
        )
        ...
        if debug:
            if device == "cpu":
                np.save(join(am_output, f"{i[:-out_format]}_coord_voxel.npy"), point_cloud)
