...
@click.option("-lr", "--loss_lr", default=1.0, type=float, help="Learning rate.", show_default=True)
@click.option(
    "-lrs",
    "--lr_rate_schedule",
    default=False,
    ...
    if dataset_type.endswith("rgb"):
        if node_dim == 0:
            TardisError(
                "161",
                "tardis/train_DIST.py",
                "Model initiated with node feasters as RGB but " f"node_dim is {node_dim}.",
            )
            sys.exit()
    if node_dim > 0:
        node_input = 3
