...
class NumpyToAmira:
    ...
    def check_3d(self, coord: Optional[np.ndarray] = List) -> Union[List[np.ndarray], np.ndarray]:
        """
    ...
    def _build_header(self, coord: np.ndarray, file_dir: str, label: Optional[list] = None):
        """
    ...
    def _write_to_amira(data: list, file_dir: str):
        ...
        if not file_dir.endswith(".am"):
            TardisError("133", "tardis/utils/export_data.py", f"{file_dir} must be and .am file!")
    ...
    def export_amira(
        ...
        # Build Amira header
        self._build_header(coord=coords, file_dir=file_dir, label=self._build_labels(labels))
