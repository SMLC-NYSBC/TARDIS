...
@click.option(
    ...
    default=getcwd(),
    type=str,
    help="Directory with spatial graphs from amira and tardis.",
    show_default=True,
)
@click.option(
    "-o",
...
def main(
    ...
    if not isdir(output):
        ...
        if click.prompt(f"Remove all files in {output} ?", type=click.Choice(["y", "n"])) == "y":
            rmtree(output)
            mkdir(output)
        else:
    ...
    compare_spline = SpatialGraphCompare(
        distance_threshold=distance_threshold, interaction_threshold=interaction_threshold
    )
    ...
    for id, i in enumerate(amira_files):
        ...
        if amira_px == tardis_px:
            ...
            with open(join(output, "log.txt"), "a+") as f:
                f.write("\n" f"{datetime.now()}" f"Skipped comparison - not matching pixel size!")
