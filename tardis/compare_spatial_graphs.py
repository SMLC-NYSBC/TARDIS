...
@click.option('-o', '--output',
              ...
              show_default=True)
@click.option('-pf_a', '--amira_prefix',
              default=None,
              type=str,
              help='Prefix name for amira spatial graph.',
              show_default=True)
@click.option('-pf_t', '--tardis_prefix',
              default=None,
              type=str,
...
def main(distance_threshold: int,
    ...
    if not isdir(output):
        ...
        if click.prompt(f'Remove all files in {output} ?',
                        type=click.Choice(['y', 'n'])) == 'y':
            rmtree(output)
            mkdir(output)
        else:
    ...
    if dir is None:
        TardisError(id='122',
                    ...
                    desc='Indicated Amira and Tardis prefixes but '
                         'not directory!')
