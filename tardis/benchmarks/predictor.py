...
class CnnBenchmark:
    ...
    def _benchmark(self, logits: np.ndarray, target: np.ndarray):
        ...
        # ACC, Prec, Recall, F1
        accuracy_score, precision_score, recall_score, F1_score = calculate_f1(input, target, False)
    ...
    def __call__(self) -> dict:
        ...
        self.tardis_progress(
            title=self.title,
            text_1=f"Running image segmentation benchmark on " f"{self.data_set} dataset",
