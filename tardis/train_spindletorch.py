...
@click.option(
    ...
    default=getcwd(),
    type=str,
    help="Directory with train, test folder or folder with dataset " "to be used for training.",
    show_default=True,
)
@click.option("-ps", "--patch_size", default=64, type=int, help="Image size used for prediction.", show_default=True)
@click.option(
    "-px",
    "--pixel_size",
    default=23.2,
...
@click.option("-ms", "--mask_size", default=150, type=int, help="Size of drawn mask in A.", show_default=True)
@click.option(
    "-cnn",
    "--cnn_type",
    default="unet",
...
@click.option(
    "-co", "--cnn_out_channel", default=1, type=int, help="Number of output channels for the NN.", show_default=True
)
@click.option("-b", "--training_batch_size", default=25, type=int, help="Batch size.", show_default=True)
@click.option("-cl", "--cnn_layers", default=5, type=int, help="Number of convolution layer for NN.", show_default=True)
@click.option(
    "-cm", "--cnn_scaler", default=64, type=int, help="Convolution multiplayer for CNN layers.", show_default=True
)
@click.option(
    "-cs",
    "--cnn_structure",
...
@click.option(
    "-ck", "--conv_kernel", default=3, type=int, help="Kernel size for 2D or 3D convolution.", show_default=True
)
@click.option("-cp", "--conv_padding", default=1, type=int, help="Padding size for convolution.", show_default=True)
@click.option("-cmpk", "--pool_kernel", default=2, type=int, help="Max_pooling kernel.", show_default=True)
@click.option(
    "-l", "--cnn_loss", default="BCELoss", type=str, help="Loss function use for training.", show_default=True
)
@click.option("-lr", "--loss_lr_rate", default=1.0, type=float, help="Learning rate for NN.", show_default=True)
@click.option(
    "-lrs",
    "--lr_rate_schedule",
    default=False,
...
@click.option("-w", "--warmup", default=100, type=int, help="Number of warmup steps.", show_default=True)
@click.option("-e", "--epochs", default=100, type=int, help="Number of epoches.", show_default=True)
@click.option(
    "-es",
    "--early_stop",
    default=10,
    type=int,
    help="Number of epoches without improvement after which early stop " "is initiated.",
    show_default=True,
)
@click.option(
    "-cch",
...
@click.option(
    "-dp", "--dropout_rate", default=None, type=float, help="If indicated, value of dropout for CNN.", show_default=True
)
...
def main(
    ...
    if not DATASET_TEST:
        ...
        build_train_dataset(
            dataset_dir=dir, circle_size=mask_size, resize_pixel_size=pixel_size, trim_xy=patch_size, trim_z=patch_size
        )
    ...
    train_DL = DataLoader(
        dataset=CNNDataset(
            img_dir=TRAIN_IMAGE_DIR, mask_dir=TRAIN_MASK_DIR, size=patch_size, out_channels=cnn_out_channel
        ),
    ...
    test_DL = DataLoader(
        dataset=CNNDataset(
            img_dir=TEST_IMAGE_DIR, mask_dir=TEST_MASK_DIR, size=patch_size, out_channels=cnn_out_channel
        ),
        shuffle=True,
        num_workers=8,
        pin_memory=True,
